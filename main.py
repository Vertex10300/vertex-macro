...
def safe_filename(name):
    ...
    return (safe or f'capture_{int(time.time())}') + ext


def new_rule(trigger, action='center', coords=None, click_targets=None):
    return {'trigger': trigger, 'action': action,
            'coords': coords or [], 'click_targets': click_targets or []}


# ===== 설정 =====
class Config:
    DEFAULT_HOTKEYS = {
        'add_pos': 'f1', 'toggle_detect': 'f2', 'toggle_repeat': 'f3',
        'switch_mode': 'f8', 'stop_all': 'f9', 'exit': 'esc',
    }

    def __init__(self):
        self.detect_rules = []
        self.click_positions_repeat = []
        self.scan_regions = []
        self.interval = 0.03
        self.confidence = 0.8
        self.repeat_delay = 1.0
        self.detect_cooldown = 0.5
        self.hotkeys = dict(self.DEFAULT_HOTKEYS)
        self.always_on_top = True
        self.detect_running = False
        self.repeat_running = False
        self.input_mode = 'detect'

    def save(self):
        data = {
            'detect_rules': self.detect_rules,
            'click_positions_repeat': self.click_positions_repeat,
            'scan_regions': self.scan_regions,
            'confidence': self.confidence, 'interval': self.interval,
            'repeat_delay': self.repeat_delay, 'detect_cooldown': self.detect_cooldown,
            'hotkeys': self.hotkeys, 'always_on_top': self.always_on_top,
        }
        try:
    ...
    def load(self):
        ...
        self.detect_rules = data.get('detect_rules', [])
        # 구 형식 호환
        if not self.detect_rules and 'detect_images' in data:
            old_mode = data.get('detect_click_mode', 'coord')
            old_coords = [tuple(p) for p in data.get('click_positions_detect', [])]
            old_imgs = data.get('click_images', [])
            for fname in data['detect_images']:
                r = new_rule(fname)
                if old_mode == 'coord' and old_coords:
                    r['action'], r['coords'] = 'coord', list(old_coords)
                elif old_mode == 'image' and old_imgs:
                    r['action'], r['click_targets'] = 'image', list(old_imgs)
                self.detect_rules.append(r)

        self.detect_rules = [r for r in self.detect_rules if os.path.exists(img_path(r['trigger']))]
        for r in self.detect_rules:
            r['click_targets'] = [f for f in r.get('click_targets', []) if os.path.exists(img_path(f))]
            r['coords'] = [tuple(c) for c in r.get('coords', [])]

        self.click_positions_repeat = [tuple(p) for p in data.get('click_positions_repeat', [])]
        self.scan_regions = [tuple(r) for r in data.get('scan_regions', [])]
        self.confidence = data.get('confidence', 0.8)
        self.interval = data.get('interval', 0.03)
        self.repeat_delay = data.get('repeat_delay', 1.0)
        self.detect_cooldown = data.get('detect_cooldown', 0.5)
...
class ImageMatcher:
    ...
    def _load_template(self, img_file):
        path = img_path(img_file)
        if not os.path.exists(path):
            return None
        mtime = os.path.getmtime(path)
        cached = self._tpl_cache.get(img_file)
        if cached and cached[0] == mtime:
            return cached[1]
        try:
            arr = np.fromfile(path, dtype=np.uint8)
            tpl = cv2.imdecode(arr, cv2.IMREAD_COLOR)
        except Exception:
            return None
        if tpl is not None:
            self._tpl_cache[img_file] = (mtime, tpl)
            self._mask_cache.pop(img_file, None)
        return tpl

    def _get_edge_mask(self, img_file, image):
        cached = self._mask_cache.get(img_file)
        if cached is not None:
            return cached
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 50, 150)
        mask = cv2.dilate(edges, np.ones((3, 3), np.uint8), iterations=2) > 0
        result = mask if np.count_nonzero(mask) > 10 else None
        self._mask_cache[img_file] = result
        return result
    ...
    def _match_template(self, screen, template, confidence):
        ...
        best_val, best_loc, best_h, best_w = 0, None, h, w

        for scale in (1.0, 0.95, 1.05, 0.9, 1.1):
            nw, nh = int(w * scale), int(h * scale)
            if nw < 5 or nh < 5 or nw > sw or nh > sh:
                continue
            t = template if scale == 1.0 else cv2.resize(template, (nw, nh), interpolation=cv2.INTER_AREA)
            result = cv2.matchTemplate(screen, t, cv2.TM_CCOEFF_NORMED)
            _, max_val, _, max_loc = cv2.minMaxLoc(result)
            if max_val > best_val:
                best_val, best_loc, best_w, best_h = max_val, max_loc, nw, nh
            # 1.0 스케일에서 충분히 높으면 나머지 스킵
            if scale == 1.0 and max_val >= min(confidence + 0.1, 0.98):
                break

        return (best_val, best_loc, best_w, best_h) if best_val >= confidence else None

    def _verify_color(self, img_file, template, screen, loc, w, h):
        matched = screen[loc[1]:loc[1]+h, loc[0]:loc[0]+w]
        if matched.shape[:2] != (h, w):
            return True
        th, tw = template.shape[:2]
        tpl = cv2.resize(template, (w, h), interpolation=cv2.INTER_AREA) if (h, w) != (th, tw) else template
        # 리사이즈된 경우 캐시 사용 불가, 원본 크기일 때만 캐시
        if (h, w) == (th, tw):
            mask = self._get_edge_mask(img_file, tpl)
        else:
        ...
        if mask is None:
            return True
        diff = np.mean(np.abs(tpl[mask].astype(np.float32) - matched[mask].astype(np.float32)))
        return diff <= 30

    def locate(self, img_file, confidence=0.8, region=None, screen=None):
        template = self._load_template(img_file)
        if template is None:
            return None
        if screen is None:
            screen = self.grab(region)
        match = self._match_template(screen, template, confidence)
        if match is None:
            return None
        _, loc, fw, fh = match
        if not self._verify_color(img_file, template, screen, loc, fw, fh):
            return None
...
# ===== 매크로 루프 =====
def detect_loop():
    while cfg.detect_running:
        if not cfg.detect_rules:
            time.sleep(0.5)
            continue
        try:
            found = False
            regions = cfg.scan_regions or [None]
            for rule in cfg.detect_rules:
                if not cfg.detect_running:
                    break
                ...
                for region in regions:
                    if not cfg.detect_running:
                        break
                    try:
                        screen = matcher.grab(region)
                        loc = matcher.locate(trigger, cfg.confidence, region, screen)
                        if not loc:
                            continue
                        found = True
                        ...
                        if action == 'image' and rule.get('click_targets'):
                            _run_image_chain(trigger, rule['click_targets'])
                        elif action == 'coord' and rule.get('coords'):
                            _run_coord_chain(trigger, rule['coords'])
                        else:
                            pyautogui.click(loc[4], loc[5])
                            app.log(f'{trigger} → 중앙 클릭')

                        time.sleep(cfg.detect_cooldown)
                        while cfg.detect_running:
                            if not matcher.locate(trigger, cfg.confidence, region):
                                break
...
def _run_image_chain(trigger, targets):
    total = len(targets)
    for ci, ct in enumerate(targets):
        if not cfg.detect_running:
            return
        ct_loc = None
        for _ in range(int(5 / max(cfg.interval, 0.01))):
            if not cfg.detect_running:
                return
            ct_loc = matcher.locate(ct, cfg.confidence, screen=matcher.grab())
            if ct_loc:
                break
            time.sleep(cfg.interval)
        if not ct_loc:
        ...
        app.log(f'{trigger} → [{ci+1}/{total}] {ct} 클릭')
    app.log(f'{trigger} → 전체 {total}개 완료')


def _run_coord_chain(trigger, coords):
    total = len(coords)
    for ci, pos in enumerate(coords):
        if not cfg.detect_running:
            break
        pyautogui.click(*pos)
        app.log(f'{trigger} → [{ci+1}/{total}] ({pos[0]}, {pos[1]}) 클릭')
        if ci < total - 1:
            time.sleep(cfg.detect_cooldown)


def repeat_loop():
    while cfg.repeat_running:
        for i, pos in enumerate(cfg.click_positions_repeat):
            if not cfg.repeat_running:
                break
            pyautogui.click(*pos)
            app.log(f'반복 {i+1}: ({pos[0]}, {pos[1]})')
            time.sleep(cfg.repeat_delay)


# ===== 테마 =====
THEME = {
    'bg': '#0d0d0d', 'surface': '#161616', 'card': '#1f1f1f',
    'border': '#2a2a2a', 'accent': '#ffffff', 'accent_dim': '#888888',
    'text': '#e0e0e0', 'text_dim': '#666666',
...
class MacroApp:
    ...
    def _setup_styles(self):
        ...
        for name, fg, font in [
            ('TLabel', t['text'], ('Segoe UI', 9)),
            ('H1.TLabel', t['accent'], ('Segoe UI', 13, 'bold')),
            ('H2.TLabel', t['text'], ('Segoe UI', 10, 'bold')),
            ('Dim.TLabel', t['text_dim'], ('Segoe UI', 8)),
            ('On.TLabel', t['green'], ('Segoe UI', 9, 'bold')),
            ('Off.TLabel', t['red'], ('Segoe UI', 9, 'bold')),
        ]:
            s.configure(name, background=t['bg'], foreground=fg, font=font)
        ...
        s.configure('Accent.TButton', font=('Segoe UI', 9, 'bold'), padding=[10, 5],
        ...
        s.configure('Danger.TButton', font=('Segoe UI', 9), padding=[8, 4],
                     background=t['red'], foreground='white', bordercolor=t['red'])
        s.map('Danger.TButton',
              background=[('active', '#dc2626'), ('pressed', '#b91c1c')],
              foreground=[('active', 'white'), ('pressed', 'white')])

        s.configure('TNotebook', background=t['bg'], borderwidth=0, tabmargins=[0, 0, 0, 0])
        s.configure('TNotebook.Tab', background=t['surface'], foreground=t['text_dim'],
        ...
        for w in ('TRadiobutton', 'TCheckbutton'):
            s.configure(w, background=t['bg'], foreground=t['text'], font=('Segoe UI', 9))
    ...
    def _build_tab_rules(self, nb):
        ...
        self.detect_btn = ttk.Button(top, text='▶ 시작', style='Accent.TButton', command=self.toggle_detect)
        self.detect_btn.pack(side='left', padx=(0, 8))
        for text, cmd in [('캡처 추가', self.capture_trigger), ('파일 추가', self.add_trigger_file), ('삭제', self.del_rule)]:
        ...
        for txt, val in [('감지된 이미지 중앙 클릭', 'center'), ('지정 좌표 클릭', 'coord'), ('다른 이미지 찾아서 클릭', 'image')]:
        ...
        for text, cmd in [('+ 마우스(F1)', self._rule_add_coord_mouse), ('+ 직접입력', self._rule_add_coord_manual), ('삭제', self._rule_del_coord)]:
            ttk.Button(rc_btn, text=text, command=cmd).pack(side='left', padx=2)
        ...
        for text, cmd in [('캡처', self._rule_capture_click_img), ('파일 추가', self._rule_add_click_img_file), ('삭제', self._rule_del_click_img)]:
            ttk.Button(ri_btn, text=text, command=cmd).pack(side='left', padx=2)
    ...
    def _build_tab_repeat(self, nb):
        ...
        for text, cmd in [('+ 마우스', self.add_pos_mouse), ('+ 직접입력', self.add_pos_manual),
                          ('삭제', self.del_repeat_pos), ('초기화', self.clear_repeat_pos)]:
    ...
    def _build_tab_region(self, nb):
        ...
        for text, cmd, style in [('화면 선택', self.start_region, 'Accent.TButton'),
                                  ('+ 직접입력', self.add_region_manual, 'TButton'),
                                  ('삭제', self.del_region, 'TButton'),
                                  ('초기화', self.clear_regions, 'TButton')]:
            ttk.Button(btn, text=text, style=style, command=cmd).pack(side='left', padx=2)
    ...
    def _on_region_select(self, event=None):
        ...
        if not sel or sel[0] >= len(cfg.scan_regions):
            return
        r = cfg.scan_regions[sel[0]]
        self._show_region_highlight(*r)

    def _show_region_highlight(self, x, y, w, h):
        if self._region_highlight:
            try: self._region_highlight.destroy()
        ...
        c.create_text(w // 2, h // 2, text=f'{w} x {h}', fill='#ffffff', font=('Segoe UI', 14, 'bold'))
    ...
    def _build_tab_settings(self, nb):
        ...
        fields = [('스캔 간격 (초)', 'interval_var', str(cfg.interval)),
                  ('반복 간격 (초)', 'repeat_delay_var', str(cfg.repeat_delay)),
                  ('감지 후 대기 (초)', 'cooldown_var', str(cfg.detect_cooldown)),
                  ('정확도 (0~1)', 'confidence_var', str(cfg.confidence))]
        ...
        for k, label in hk_labels.items():
            ...
            var = tk.StringVar(value=cfg.hotkeys.get(k, ''))
    ...
    def refresh_all(self):
        ...
        for i, r in enumerate(cfg.detect_rules):
            self.rule_list.insert('end', f"  {i+1}.  {r['trigger']}")
        if self._selected_rule_idx is not None and self._selected_rule_idx < len(cfg.detect_rules):
        ...
        for i, p in enumerate(cfg.click_positions_repeat):
            self.repeat_list.insert('end', f'  {i+1}.  ({p[0]}, {p[1]})')

        self.region_list.delete(0, 'end')
        for i, r in enumerate(cfg.scan_regions):
    ...
    def _refresh_rule_detail(self):
        if self._selected_rule_idx is not None and self._selected_rule_idx < len(cfg.detect_rules):
            rule = cfg.detect_rules[self._selected_rule_idx]
    ...
    def update_status(self):
        on, off = cfg.detect_running, not cfg.detect_running
        self.detect_status.config(text='감지 ON' if on else '감지 OFF', style='On.TLabel' if on else 'Off.TLabel')
        self.detect_btn.config(text='■ 중지' if on else '▶ 시작')
        on = cfg.repeat_running
        self.repeat_status.config(text='반복 ON' if on else '반복 OFF', style='On.TLabel' if on else 'Off.TLabel')
        self.repeat_btn.config(text='■ 중지' if on else '▶ 시작')
        self.mode_label.config(text=f'F1 대상: {cfg.input_mode}')
    ...
    def _on_rule_select(self, event=None):
        ...
        if not sel or sel[0] >= len(cfg.detect_rules):
            return
        idx = sel[0]
        if self._selected_rule_idx is not None and self._selected_rule_idx < self.rule_list.size():
            self.rule_list.itemconfig(self._selected_rule_idx, bg=THEME['surface'], fg=THEME['text'])
        self._selected_rule_idx = idx
        rule = cfg.detect_rules[idx]
    ...
    def _on_rule_action_change(self):
        ...
        cfg.detect_rules[self._selected_rule_idx]['action'] = action
        cfg.save()
        self.refresh_all()
    ...
    def _import_image(self, path):
        ...
        if os.path.normpath(path).startswith(os.path.normpath(IMAGES_DIR) + os.sep):
            return name
    ...
    def capture_trigger(self):
        def on_done(screenshot, x, y, w, h):
            ...
            cfg.detect_rules.append(new_rule(filename))
            cfg.save()
            self.refresh_all()
    ...
    def add_trigger_file(self):
        ...
        for p in paths:
            ...
            if not any(r['trigger'] == fname for r in cfg.detect_rules):
                cfg.detect_rules.append(new_rule(fname))
                added += 1
        if added:
            cfg.save()
            self.refresh_all()
    ...
    def _get_selected_rule(self):
        if self._selected_rule_idx is not None and self._selected_rule_idx < len(cfg.detect_rules):
            return cfg.detect_rules[self._selected_rule_idx]
        return None
    ...
    def _rule_add_coord_manual(self):
        rule = self._get_selected_rule()
        if rule:
            self._show_coord_dialog(lambda x, y: (rule['coords'].append((x, y)), cfg.save(), self._refresh_rule_detail()))
    ...
    def _rule_capture_click_img(self):
        ...
        def on_done(screenshot, x, y, w, h):
            cropped = screenshot.crop((x, y, x + w, y + h))
            filename = f'click_{time.strftime("%Y%m%d_%H%M%S")}.png'
            cropped.save(img_path(filename))
    ...
    def _open_overlay(self, guide_text, outline_color, on_done):
        ...
        def on_press(e):
            ...
            rect_id[0] = canvas.create_rectangle(start[0], start[1], start[0], start[1], outline=outline_color, width=2)
        def on_drag(e):
            if rect_id[0]:
        ...
        overlay.bind('<Escape>', lambda e: self._cancel_overlay())
    ...
    def _show_coord_dialog(self, callback):
        ...
        def ok(event=None):
            try:
                callback(int(xe.get()), int(ye.get()))
                dialog.destroy()
            except ValueError:
    ...
    def add_pos_mouse(self):
        x, y = pyautogui.position()
        cfg.click_positions_repeat.append((x, y))
        cfg.save()
        self.refresh_all()
    ...
    def del_repeat_pos(self):
        ...
        if sel:
            cfg.click_positions_repeat.pop(sel[0])
            cfg.save()
            self.refresh_all()

    def clear_repeat_pos(self):
        cfg.click_positions_repeat.clear()
        cfg.save()
        self.refresh_all()
        self.log('반복 좌표 초기화')
    ...
    def start_region(self):
        def on_done(_ss, x, y, w, h):
            cfg.scan_regions.append((x, y, w, h))
            cfg.save()
            self.refresh_all()
    ...
    def add_region_manual(self):
        ...
        def ok(event=None):
            try:
                cfg.scan_regions.append(tuple(int(e.get()) for e in entries))
                cfg.save()
                self.refresh_all()
                dialog.destroy()
            except ValueError:
    ...
    def del_region(self):
        sel = self.region_list.curselection()
        if sel:
            cfg.scan_regions.pop(sel[0])
            cfg.save()
            self.refresh_all()

    def clear_regions(self):
        cfg.scan_regions.clear()
        cfg.save()
        self.refresh_all()
        self.log('영역 초기화')
    ...
    def apply_settings(self):
        try:
            cfg.interval = float(self.interval_var.get())
            cfg.repeat_delay = float(self.repeat_delay_var.get())
            cfg.detect_cooldown = float(self.cooldown_var.get())
            cfg.confidence = float(self.confidence_var.get())
        except ValueError:
        ...
        for k, var in self.hotkey_vars.items():
            ...
            new_hk[k] = v
        cfg.hotkeys = new_hk
        cfg.always_on_top = self.topmost_var.get()
        self.root.attributes('-topmost', cfg.always_on_top)
        cfg.save()
        self._update_hotkey_label()
        self.log('설정 적용')
    ...
    def reset_settings(self):
        if not messagebox.askyesno('확인', '설정 초기화?'):
            return
        cfg.interval, cfg.repeat_delay, cfg.detect_cooldown, cfg.confidence = 0.03, 1.0, 0.5, 0.8
        cfg.hotkeys = dict(Config.DEFAULT_HOTKEYS)
    ...
    def toggle_detect(self):
        if not cfg.detect_running:
            if not cfg.detect_rules:
                ...
                return
            cfg.detect_running = True
            threading.Thread(target=detect_loop, daemon=True).start()
            self.log('감지 시작')
        else:
            cfg.detect_running = False
            self.log('감지 중지')
        self.update_status()

    def toggle_repeat(self):
        if not cfg.repeat_running:
            if not cfg.click_positions_repeat:
                messagebox.showwarning('경고', '반복 좌표를 먼저 추가하세요')
                return
            cfg.repeat_running = True
            threading.Thread(target=repeat_loop, daemon=True).start()
            self.log('반복 시작')
        else:
            cfg.repeat_running = False
            self.log('반복 중지')
        self.update_status()
    ...
    def on_key(self, key):
        matched = None
        for action, key_name in cfg.hotkeys.items():
            try:
        ...
        try:
            if matched == 'add_pos':
                ...
                cfg.input_mode = 'repeat' if cfg.input_mode == 'detect' else 'detect'
                self.root.after(0, self.update_status)
                self.log(f'입력 대상: {cfg.input_mode}')
